...
export const scrapeStory = async () => {
    ...
    try {
        ...
        $('tr.athing.submission').each((index, element) => {
            ...
            const dateOfTimestampFromString = new Date(dateString + 'Z');
            // console.log(`Date object from dateString: ${dateOfTimestampFromString}`);
            // console.log(`the type of dateOfTimestampFromString -- ${typeof dateOfTimestampFromString}`);
            ...
            // console.log(`this was posted this many hours ago by dateOfTimestampFromString : ${(Date.now() - dateOfTimestampFromString.getTime()) / 1000 / 60 / 60}`);
